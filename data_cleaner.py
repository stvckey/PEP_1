...
with open('./data_dirty/customer_demographics.yaml', 'r') as stream:
    data = yaml.safe_load(stream)


# each entry contains the same order and name for the keys
# creating a pandas df with the key:value pair of each entry as 
# the column:value
...
def reformat_phone_number(phone):
    ...
    if len(all_digits) > 10 and all_digits.startswith(
        '0') and all_digits[2] == '1':
    ...
    formatted_number = '{}-{}-{}{}'.format(all_digits[:3],
                                           all_digits[3:6],
                                           all_digits[6:10], ext)
...
# applying functions to demographics data
demo_df['phone_number'] = demo_df['phone_number'].apply(reformat_phone_number)

demo_df['credit_card_provider'] = demo_df['credit_card_provider'].apply(
    remove_numbers_and_digit)


# dropping duplicate entries of emails
demo_df['email'] = demo_df['email'].drop_duplicates(keep='first')


# changing customer_id to demogrpahics ID and creating the customer ID's 
# based on the index
demo_df.rename(columns={"customer_id":"customer_demographic_id"},inplace=True)
demo_df['customer_id'] = demo_df.index


# re-organizing demographics columns to match schema
...
## Commented out to wait to see if client wants to keep customer names 
# # with no contact info
# demo_df.dropna(subset=['address','email','phone_number'],how='all',inplace=True)


# loading data from orders.csv
orders_df = pd.read_csv("./data_dirty/orders.csv", encoding="utf-8")


# creating customer statistics dataframe to be used as 
# the customer_statistics table
...
# exporting dataframes to csv format
...
stat_columns = customer_stats.columns
customer_table = final_merge.loc[:, final_merge.columns.isin(stat_columns)
